...
module RSpecTracer
  class Reporter
    ...
    def write_all_examples_report
      ...
      File.write(file_name, RSpecTracer.cache_serializer.serialize(@all_examples), encoding: RSpecTracer.cache_serializer::ENCODING)
    end

    def write_flaky_examples_report
      file_name = File.join(@cache_dir, "flaky_examples.#{RSpecTracer.cache_serializer::EXTENSION}")

      File.write(file_name, RSpecTracer.cache_serializer.serialize(@flaky_examples.to_a), encoding: RSpecTracer.cache_serializer::ENCODING)
    end

    def write_failed_examples_report
      file_name = File.join(@cache_dir, "failed_examples.#{RSpecTracer.cache_serializer::EXTENSION}")

      File.write(file_name, RSpecTracer.cache_serializer.serialize(@failed_examples.to_a), encoding: RSpecTracer.cache_serializer::ENCODING)
    end

    def write_pending_examples_report
      file_name = File.join(@cache_dir, "pending_examples.#{RSpecTracer.cache_serializer::EXTENSION}")

      File.write(file_name, RSpecTracer.cache_serializer.serialize(@pending_examples.to_a), encoding: RSpecTracer.cache_serializer::ENCODING)
    end

    def write_all_files_report
      file_name = File.join(@cache_dir, "all_files.#{RSpecTracer.cache_serializer::EXTENSION}")

      File.write(file_name, RSpecTracer.cache_serializer.serialize(@all_files), encoding: RSpecTracer.cache_serializer::ENCODING)
    end

    def write_dependency_report
      file_name = File.join(@cache_dir, "dependency.#{RSpecTracer.cache_serializer::EXTENSION}")

      File.write(file_name, RSpecTracer.cache_serializer.serialize(@dependency), encoding: RSpecTracer.cache_serializer::ENCODING)
    end
    ...
    def write_reverse_dependency_report
      ...
      File.write(file_name, RSpecTracer.cache_serializer.serialize(@reverse_dependency), encoding: RSpecTracer.cache_serializer::ENCODING)
    end

    def write_examples_coverage_report
      file_name = File.join(@cache_dir, "examples_coverage.#{RSpecTracer.cache_serializer::EXTENSION}")

      File.write(file_name, RSpecTracer.cache_serializer.serialize(@examples_coverage), encoding: RSpecTracer.cache_serializer::ENCODING)
    end
    ...
    def write_last_run_report
      ...
      File.write(file_name, RSpecTracer.cache_serializer.serialize(last_run_data), encoding: RSpecTracer.cache_serializer::ENCODING)
    end
  end
end
